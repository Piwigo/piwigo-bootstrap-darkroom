...
add_event_handler('loc_begin_picture', 'get_all_pictures_in_category');
function get_all_pictures_in_category()
{
  global $template, $conf, $user, $page;
  ...
  $template->assign( array(
    'derivative_params' => trigger_change('get_index_derivative_params', ImageStdParams::get_by_type( pwg_get_session_var('index_deriv', IMG_LARGE) ) ),
    'maxRequests' =>$conf['max_requests'],
    'SHOW_THUMBNAIL_CAPTION' =>$conf['show_thumbnail_caption'],
      ) );
  $tpl_thumbnails_var = trigger_change('loc_end_index_thumbnails', $tpl_thumbnails_var, $pictures);
  $template->assign('pictures', $tpl_thumbnails_var);
